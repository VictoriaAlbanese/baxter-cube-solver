...
vector<Face> make_boring_test_face_vector(); 
...
void RotateLeftCW_CalledNormally_CubeIsRotatedCorrectly() 
{
	...
	// Rotate the front face
	interesting_face_cube.rotate_left_face_cw();
...
void RotateLeftCCW_CalledNormally_CubeIsRotatedCorrectly() 
{
	...
	// Rotate the front face
	interesting_face_cube.rotate_left_face_ccw();
...
// Make Boring Test Face Vector
// Arguments: n/a
// Returns: A color matrix which represents that needed to create a test face
// Purpose: Refactors making the same face every time for testing (solved cube)
vector<Face> make_boring_test_face_vector() 
{
	vector<Face> faces;
	vector< vector<int> > face_color_matrix;
